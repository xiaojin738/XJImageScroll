...
@implementation XJImageScroll
{
    ...
    NSTimer *timer; //计时器
    NSInteger scrollIndex;  // 下标值
    NSDictionary *imgCacheDic;  // 图片缓存
}

+ (UIView *)showLocationImageScrollWithFrame:(CGRect)rect andImageArray:(NSArray *)array andBtnClick:(BtnClickBlock)btnClick
{
    XJImageScroll *selfImageScroll = [[XJImageScroll alloc] initWithFrame:rect];
...
{
    [[XJImageScroll alloc] clearNetImageCache];
}
...
- (void)networkImageSettingWithString:(NSString *)str andShowImageView:(UIButton *)aBtn
{
    ...
    NSData *imgData = [imgCacheDic objectForKey:md5Str];
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:path]) {
        ...
        [aBtn setImage:img forState:UIControlStateNormal];
        [imgCacheDic setValue:UIImagePNGRepresentation(img) forKey:@"md5Str"];
    } else {
